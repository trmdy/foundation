...
13a. **Document identity and addressing** (raised by multiplayer walkthrough,
    2026-07-31): sync without a shared filesystem/git path needs a stable document
    id minted at creation, carried in the `fdn-doc` header and the chain, so peers
    and relays can pair chains. Same decision surface as the project manifest
    (members, design-system inheritance, matrix defaults) — decide together, before
    Phase B's gateway verbs fix an addressing scheme. Related: CLI author identity
    must default per-person (hive identity or user:<name>@<host>), never a shared
    literal — peer ids derive from author strings and shared defaults collide.
    Merge reporting: chain merges should surface concurrent overlapping property
    writes as review signals (resolved deterministically, never silently ideal).
